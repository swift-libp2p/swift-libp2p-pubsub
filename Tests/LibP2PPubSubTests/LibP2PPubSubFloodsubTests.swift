//
//  LibP2PPubSubFloodsubTests.swift
//  
//
//  Created by Brandon Toms on 4/25/22.
//
...
class LibP2PPubSubFloodsubTests: XCTestCase {
    ...
    func testLibP2PPubSub_FloodSub() throws {
        ...
        let subscription1 = try node1.pubsub.floodsub.subscribe(.init(topic: "fruit", signaturePolicy: .strictSign, validator: .acceptAll, messageIDFunc: .hashSequenceNumberAndFromFields))
        ...
        let subscription2 = try node2.pubsub.floodsub.subscribe(.init(topic: "fruit", signaturePolicy: .strictSign, validator: .acceptAll, messageIDFunc: .hashSequenceNumberAndFromFields))
    ...
    func testLibP2PPubSub_FloodSub_Subscriptions() throws {
        ...
        /// Node1 subscribes to topic 'fruit'
        let subscription1 = try node1.pubsub.floodsub.subscribe(.init(topic: "news", signaturePolicy: .strictSign, validator: .acceptAll, messageIDFunc: .hashSequenceNumberAndFromFields))
        subscription1.on = { event -> EventLoopFuture<Void> in
            switch event {
            case .newPeer(let peer):
                node1.logger.info("Node1::NewPeer -> \(peer)")
        ...
        var subscription2 = try node2.pubsub.floodsub.subscribe(.init(topic: "news", signaturePolicy: .strictSign, validator: .acceptAll, messageIDFunc: .hashSequenceNumberAndFromFields))
    ...
    func testLibP2PPubSub_FloodSub_NNodes() throws {
        ...
        case .linear:
            ...
            try nodes.enumerated().forEach { (idx, node) in
                ...
                try node.libp2p.newStream(to: nodes[idx + 1].libp2p.listenAddresses.first!, forProtocol: FloodSub.multicodec)
            }
        ...
        case .beacon2beacon:
            ...
            try nodes.enumerated().forEach { (idx, node) in
                ...
                if idx % 2 == 0 {
                    ...
                    try node.libp2p.newStream(to: nodes[1].libp2p.listenAddresses.first!, forProtocol: FloodSub.multicodec)
                }
            }
...
//                }
        ...
        do {
            ...
            try app.newStream(to: Multiaddr("/ip4/139.178.88.229/tcp/4001/p2p/12D3KooWK3rWCYssQkQHHm5q1K1qHUBRgmEp18sHDnxRRtL5kPsb"), forProtocol: "/ipfs/ping/1.0.0")
        } catch {
            print("\(error)")
        }
