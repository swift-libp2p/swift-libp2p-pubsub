...
//
//
//  Created by Brandon Toms on 7/28/21.
//

import LibP2P

fileprivate protocol RPCValidator {
...
open class BasePubSub {
    ...
    internal enum _PubSubEvent {
        ...
        var description:String {
            ...
            case .message(let peerID, let messages):
                return "message(\(peerID), [\(messages.map({ "(\($0.topicIds.first ?? "???") -> \(String(data: $0.data, encoding: .utf8) ?? "Not UTF8")"}).joined(separator: ", "))]"
            default:
                return "\(self)"
            }
        }
    ...
    internal init(group:EventLoopGroup, libp2p:Application, peerState:PeerStateProtocol, messageCache:MessageStateProtocol, debugName:String, multicodecs:[String], globalSignaturePolicy:PubSub.SignaturePolicy = .strictSign, canRelayMessages:Bool = false, emitSelf:Bool = false) throws {
    ...
    public func start() throws {
        ...
        libp2p.topology.register(TopologyRegistration(
...
//    }
    ...
    private func onPeerConnected(peer:PeerID, connection:Connection) {
        ...
        guard self.state == .started else { self.logger.warning("Ignoring onPeerConnected notification received while in state '\(state)'"); return }
        ...
        if let conn = connection as? BasicConnectionLight {
            self.logger.debug("PubSub::Attempting to auto-dial \(connection.remotePeer?.description ?? "nil") for outbound `\(codec)` stream")
    ...
    private func onPeerDisconnected(peer:PeerID) {
        guard self.state == .started else { self.logger.warning("Ignoring onPeerDisconnected notification received while in state '\(state)'"); return }
    ...
    public func processRequest(_ req:Request) -> EventLoopFuture<Response<ByteBuffer>> {
        ...
        case .ready:
            ...
            guard let stream = req.connection.hasStream(forProtocol: multicodecs.first!.stringValue, direction: req.streamDirection) else {
...
//    }
    ...
    private func processPubSubMessage(_ message:PubSubMessage) -> EventLoopFuture<PubSubMessage> {
        ...
        return self.messageCache.exists(messageID: id, on: nil).flatMap { exists -> EventLoopFuture<PubSubMessage> in
            guard exists == false else { self.logger.trace("Dropping Duplicate Message"); return self.eventLoop.makeFailedFuture(Errors.duplicateMessage) }
            ...
            return self.validate(message: message).flatMap { valid -> EventLoopFuture<PubSubMessage> in
                guard valid else { self.logger.warning("Dropping Invalid Message: \(message)"); return self.eventLoop.makeFailedFuture(Errors.failedMessageValidation) }
                ...
                return self.messageCache.put(messageID: id, message: (topic: message.topicIds.first!, data: message), on: nil).flatMap { valid -> EventLoopFuture<PubSubMessage> in
                    guard valid else {
    ...
    private func batchProcessPubSubMessages(_ messages:[PubSubMessage]) -> EventLoopFuture<[PubSubMessage]> {
        return self.ensureSignaturePolicyConformance(messages).flatMap { signedMessages -> EventLoopFuture<[PubSubMessage]> in
            guard !signedMessages.isEmpty else { return self.eventLoop.makeSucceededFuture([]) }
            
            /// Compute the message ID for each message
            return self.computeMessageIds(signedMessages).flatMap { identifiedMessages -> EventLoopFuture<[PubSubMessage]> in
                ...
                return self.discardKnownMessagesUsingSeenCache(identifiedMessages).flatMap { newMessages -> EventLoopFuture<[PubSubMessage]> in
                    ...
                    return self.storeMessages(newMessages).flatMap { storedMessages -> EventLoopFuture<[PubSubMessage]> in
                        
                        /// Return the new / unique messages for further processing
                        return self.eventLoop.makeSucceededFuture(storedMessages.map { $0.value })
                    }
                }
            }
        }
...
//    }
    ...
    internal func processInboundMessage(_ msg:PubSubMessage, from:PeerID, request:Request) -> EventLoopFuture<Void> {
        assertionFailure("Your PubSub implementation must override the PubSub::ProcessInboundMessage(:Data, :Stream, :LibP2P.ProtocolRequest) method")
        return request.eventLoop.makeSucceededVoidFuture()
    }
    
    /// Has to be overriden by our custom router implementations
    internal func processInboundMessages(_ messages:[PubSubMessage], from:PeerID, request:Request) -> EventLoopFuture<Void> {
        assertionFailure("Your PubSub implementation must override the PubSub::ProcessInboundMessage(:Data, :Stream, :LibP2P.ProtocolRequest) method")
        return request.eventLoop.makeSucceededVoidFuture()
    }
    ...
    func passesMessageSignaturePolicy(_ message:PubSubMessage) -> Bool {
        ...
        case .strictNoSign:
            ...
            //guard !message.hasSignature && !message.hasKey else {
                self.logger.warning("Message Signature Policy Mismatch. Current Policy == Strict No Sign and the Message is signed. Dropping Message.")
                return false
            }
            return true
        case .strictSign:
            guard !message.signature.isEmpty && !message.key.isEmpty else {
            //guard message.hasSignature && message.hasKey else {
                self.logger.warning("Message Signature Policy Mismatch. Current Policy == Strict Sign and the Message isn't signed. Dropping Message.")
                return false
            }
    ...
    func publish(msg:RPC.Message, on loop:EventLoop? = nil) -> EventLoopFuture<Void> {
        ...
        return self.getPeersSubscribed(to: topic).flatMap { subscribers -> EventLoopFuture<Void> in
            if subscribers.isEmpty { self.logger.trace("No known peers subscribed to topic: \(topic). Proceeding with message signing and local storage.") }
            ...
            do {
                ...
                if subscribers.isEmpty {
                    self.logger.warning("This message was signed and stored locally. If your attached router supports publishing past messages then the message might be published to the network eventually.")
                }
    ...
    public func subscribe(_ config:PubSub.SubscriptionConfig, on loop:EventLoop? = nil) -> EventLoopFuture<Void> {
        ...
        guard !config.topic.isEmpty, config.topic != "" else { return self.eventLoop.makeFailedFuture(Errors.invalidTopic) }
        ...
        return self.eventLoop.submit {
            ...
            //let _ = self.peerState.peersSubscribedTo(topic: config.topic, on: loop).map { subs in
                guard subs.count > 0 else { self.logger.warning("No subscribers to share subscription with"); return }
                guard let subPayload = try? self.generateSubPayload(forTopics: [config.topic]) else {
                    self.logger.warning("Failed to serialize subscription payload. Unable to alert peers of subscription")
                    return
                }
...
//    }
    ...
    internal func discardKnownMessagesUsingSeenCache(_ messages:[Data:PubSubMessage]) -> EventLoopFuture<[Data:PubSubMessage]> {
        let ids = messages.keys.map { $0 }
        return self.seenCache.filter(ids: Set(ids), returningOnly: .unknown, on: self.eventLoop).map { unknownIDs -> [Data:PubSubMessage] in
            var newMessages:[Data:PubSubMessage] = [:]
            unknownIDs.forEach { newMessages[$0] = messages[$0] }
            return newMessages
        }
    ...
    internal func sortMessagesByTopic(_ messages:[Data:PubSubMessage]) -> [String:[(id:Data, message:PubSubMessage)]] {
...
extension BasePubSub {
    ...
    public func getTopics(on loop:EventLoop? = nil) -> EventLoopFuture<[Topic]> {
        return self.peerState.topicSubscriptions(on: loop)
    }
...
//    }
    ...
    func validate(message: PubSubMessage, on loop:EventLoop? = nil) -> EventLoopFuture<Bool> {
        self.eventLoop.submit { () -> Bool in
            guard let topic = message.topicIds.first else { self.logger.warning("No message topic"); return false }
            guard let validators = self.validators[topic] else { print("Warning! No Validators found for Topic: '\(topic)'. Failing message validation by default."); return false }
    ...
    func validateExtended(message:PubSubMessage, on loop:EventLoop? = nil) -> EventLoopFuture<ValidationResult> {
        self.eventLoop.submit { () -> ValidationResult in
            guard let topic = message.topicIds.first else { self.logger.warning("No message topic"); return .reject }
            guard let validators = self.validatorsExt[topic] else { print("Warning! No ExtValidators found for Topic: '\(topic)'. Failing message validation by default."); return .reject }
    ...
    func addValidator(for topic:Topic, validator:@escaping Validator, on loop:EventLoop? = nil) -> EventLoopFuture<Void> {
    ...
    func addValidatorExtended(for topic:Topic, validator:@escaping ValidatorExtended, on loop:EventLoop? = nil) -> EventLoopFuture<Void> {
        self.eventLoop.submit { () -> Void in
    ...
    func nextMessageSequenceNumber() -> [UInt8] {
        return Array(withUnsafeBytes(of: UInt64.random(in: 0...UInt64.max).bigEndian) { $0 })
    ...
    internal func verifyMessageSignature(_ message:PubSubMessage) throws -> Bool {
        ...
        let verified = try key.isValidSignature(message.signature, for: BasePubSub.MessagePrefix + messageWithoutSignature)
