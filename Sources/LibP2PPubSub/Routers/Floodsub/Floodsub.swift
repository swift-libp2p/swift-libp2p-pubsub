...
public class FloodSub:BasePubSub, PubSubCore, LifecycleHandler {
    ...
    public init(group: EventLoopGroup, libp2p: Application, debugName: String = "Floodsub", emitSelf: Bool = false) throws {
        /// Init our PeerState
        ...
        /// Init super
        try super.init(group: group, libp2p: libp2p, peerState: peerState, messageCache: messageCache, debugName: debugName, multicodecs: [FloodSub.multicodec], globalSignaturePolicy: .strictNoSign, canRelayMessages: true, emitSelf: emitSelf)
...
//    }
    ...
    override internal func processInboundMessages(_ messages:[PubSubMessage], from:PeerID, request:Request) -> EventLoopFuture<Void> {
        messages.map {
            self.processInboundMessage($0, from: from, request: request)
        }.flatten(on: request.eventLoop)
    }
    
    override internal func processInboundMessage(_ message: PubSubMessage, from: PeerID, request: Request) -> EventLoopFuture<Void> {
        ...
        return self.peerState.peersSubscribedTo(topic: message.topicIds.first!, on: nil).flatMap { subscribers -> EventLoopFuture<Void> in
            
            guard subscribers.count > 0 else { return self.eventLoop.makeSucceededVoidFuture() }
            
            self.logger.info("Checking \(subscribers.count) `\(message.topicIds.first!)` subscribers for message propogation")
            ...
            return subscribers.map { peerStreams in
                guard peerStreams.id != from else { self.logger.info("Skipping OP"); return self.eventLoop.makeSucceededVoidFuture() }
